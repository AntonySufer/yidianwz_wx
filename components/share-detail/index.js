...
const dividerHeightBottomScale = 0.6;
const dividerHeight = 0.015;
//已参与
const joinTitleHeightScale = 0.0;
const joinNumberHeightScale = 0.0;
const joinNumberWidthScale = 0.0;
//分割线
const joinLineWidthScale = 0.21;
const joinLineHeightTopScale = 0.55;
const joinLineHeightBottomScale = 0.65;
//参与用户头像
const joinAvatarRadiusScale = 0.0;
const joinAvatarWidthBaseScale = 0.0;
const joinAvatarHeightBaseScale = 0.0;
const joinAvatarDividerScale = 0.0;
...
Component({
  ...
  methods: {
    ...
    * 下载二维码
    */
    downloadQcodeImage: function () {
      let that = this;
      wx.downloadFile({
        url: that.data.ermUrlPath,
        success: function (res) {
          that.setData({
            QRPath: res.tempFilePath
          })
         
          let adImg = that.data.adImageUrl; //本地图片不下载。直接掠过
           if(adImg.indexOf('../../img/') > -1 ){
             that.setData({
               adPath: adImg
             })
             that.drawCanvas();
    ...
    drawCanvas: function () {
      ...
      ctx.setFontSize(24);
      ctx.setTextAlign('left');
      ctx.fillText(that.data.incomeMoney == 0 ? '6888+' : that.data.incomeMoney + '+', incomeMoneyWidthScale * windowWidth, incomeMoneyHeightScale * windowHeight);
